...
TEST(MockTestAdBook2, MockTest2){
  MockClass2 mockClass2;
  EXPECT_CALL(mockClass2, add_record("jan", "san pedro", 1999, "09219285032")).Times(AtLeast(1)).WillOnce(Return(true));
  EXPECT_EQ(mockClass2.add_record("jan", "san pedro", 1999, "09219285032"), 1);
};

TEST(MockTestAdBook3, MockTest2){
  MockClass2 mockClass2;
  EXPECT_CALL(mockClass2, write_file()).Times(AtLeast(1)).WillOnce(Return(true));
  EXPECT_EQ(mockClass2.write_file(), 1);
};

TEST(MockTestAdBook4, MockTest2){
  MockClass2 mockClass2;
  EXPECT_CALL(mockClass2, print_record("jan")).Times(AtLeast(1)).WillOnce(Return(true));
